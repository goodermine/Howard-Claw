 ...
 *
 * Ported from codexUI/AnyClaw's BootstrapInstaller — follows the same extraction
 * logic as Termux's TermuxInstaller.java:
 ...
 *   5. Fix hardcoded Termux paths in apt/dpkg config
 */
    ...
    private fun fixTermuxPaths(paths: Paths) {
        ...
        if (sourcesList.exists()) {
            ...
            sourcesList.writeText(
                content
                    .replace("https://", "http://")
                    .replace("com.termux", "au.howardagent")
            )
        }

        // Fix dpkg status file references
